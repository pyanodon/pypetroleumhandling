...
local function oil_gas_select(entity, global_bypass)
	...
	if string.match(E.name, "oil%-derrick") ~= nil then
		if global.oil_to_gas == true or global_bypass == true then
			...
			local gas =
				game.surfaces["nauvis"].find_entity("natural-gas-" .. string.match(string.match(E.name, "%d+"), "[^0]"), E.position)
			if oil ~= nil then
...
script.on_event(
	...
	function(event)
		...
		elseif string.match(E.name, "bitumen%-seep") ~= nil then
			--log('hit')
			local resource =
				game.surfaces[E.surface.name].find_entities_filtered {
			...
			for r, re in pairs(resource) do
				...
				if string.match(re.name, "oil") then
					...
					ass1.active = false
					global.oil_derricks[E.unit_number] = {
						entity = E,
						base = ass1,
						drilling_fluid = ""
					}
...
script.on_event(
	...
	function(event)
		...
		if string.match(event.entity.name, "oil%-derrick") then
			--log('hit')
			local oil =
				game.surfaces["nauvis"].find_entities(
				{
		...
		elseif string.match(E.name, "seep") ~= nil then
			local entity =
				game.surfaces[E.surface.name].find_entities_filtered {position = E.position, type = "assembling-machine"}
	...
	function()
		...
		for d, drill in pairs(global.oil_derricks) do
			...
			if next(dfluid) ~= nil then
				for f, fluid in pairs(dfluid) do
					--log("hit")
					if string.match(f, "3") ~= nil then
						if fluid >= 100 then
							global.oil_derricks[d].drilling_fluid = f
							drill.base.remove_fluid {name = f, amount = 5}
							drill.entity.active = true
						elseif fluid < 100 then
							drill.entity.active = false
						end
					elseif string.match(f, "2") ~= nil then
						if fluid >= 100 then
							global.oil_derricks[d].drilling_fluid = f
							drill.base.remove_fluid {name = f, amount = 5}
							drill.entity.active = true
					...
					elseif string.match(f, "1") ~= nil then
						if fluid >= 100 then
							global.oil_derricks[d].drilling_fluid = f
							drill.base.remove_fluid {name = f, amount = 5}
							drill.entity.active = true
					...
					elseif string.match(f, "0") ~= nil then
						if fluid >= 100 then
							global.oil_derricks[d].drilling_fluid = f
							drill.base.remove_fluid {name = f, amount = 5}
							drill.entity.active = true
...
script.on_event(
	...
	function(event)
		...
		if E.name == "bitumen-seep" then
			...
			if string.match(drill_fluid, "0") ~= nil then
				spawn_chance = spawn_chance + 10
			elseif string.match(drill_fluid, "1") ~= nil then
				spawn_chance = spawn_chance + 30
			elseif string.match(drill_fluid, "2") ~= nil then
				spawn_chance = spawn_chance + 60
			elseif string.match(drill_fluid, "3") ~= nil then
